...
public class ConsoleViewer {
	...
	private static void clear(){
		try {
			...
			System.err.println("clearing the screen doesnt work!");
	...
	private void printHead(int iteration, Individual fittest){
		System.out.println("MMAS Simulator - current fitness function: "+model.getFunctionName());
		System.out.println();
	...
	private void printPheromones() {
		...
		for (int i = 0; i < pheromones.length; ) {
			...
			String whichX=" ";
			for (int elemsOnLine = 0; elemsOnLine < LINEWITH && i < pheromones.length; elemsOnLine++) {
				oneProbabilities += String.format("%."+PRECITION+"f  ", pheromones[i]);
				zeroProbabilities += String.format("%."+PRECITION+"f  ", 1-pheromones[i]);
				whichX+=String.format(" %"+((PRECITION+1)/2)+"s%-2s %"+PRECITION/2+"s", "x", subscript(i+1), "");
				i++;
			}
