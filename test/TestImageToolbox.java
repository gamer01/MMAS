...
public class TestImageToolbox {
	...
	public void testImgToMatrix() throws InterruptedException {
		BufferedImage img = new BufferedImage(2, 1,
				BufferedImage.TYPE_BYTE_GRAY);
		int[] pixels = { 30, 12 };
		img.getRaster().setPixels(0, 0, 2, 1, pixels);
	...
	public void testBitsetToMatrix() {
		...
		// System.out.println(Arrays.toString(checkMatrix[0]));
		int[][] result = controller.ImageToolbox.bitsetToMatrix(binval, 2, 1);
	...
	@Test
	public void testBitsetToArray() {
		BitSet binval = new BitSet(16);
		binval.set(0);

		int[] checkArray = { 128, 0 };// 2x2 matrix
		System.out.println(Arrays.toString(checkArray));
		int[] result = controller.ImageToolbox.bitsetToArray(binval, 2, 1);

		System.out.println(Arrays.toString(result));

		assertThat(checkArray, IsEqual.equalTo(result));
